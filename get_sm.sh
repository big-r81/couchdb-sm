...
if [ "$LTS" -eq 91 ]; then
  ...
  # EOL Spidermonkey versions
  DOWNLOAD_URL="https://archive.mozilla.org/pub/firefox/releases/115.19.0esr/source/firefox-115.19.0esr.source.tar.xz"
  # download old version from Mozilla archive server
  if [ "$DOWNLOAD" == "true" ]; then
    curl -s -L -O -J "$DOWNLOAD_URL"
  fi
  ...
  # EOL Spidermonkey versions
  DOWNLOAD_URL="https://archive.mozilla.org/pub/firefox/releases/128.8.0esr/source/firefox-128.8.0esr.source.tar.xz"
  # download old version from Mozilla archive server
  if [ "$DOWNLOAD" == "true" ]; then
    curl -s -L -O -J "$DOWNLOAD_URL"
  fi
