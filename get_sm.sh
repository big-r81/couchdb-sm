...
if [ "$LTS" -eq 91 ]; then
  ...
  # EOL Spidermonkey versions
  DOWNLOAD_URL="https://archive.mozilla.org/pub/firefox/releases/115.23.0esr/source/firefox-115.23.0esr.source.tar.xz"
  # download old version from Mozilla archive server
  if [ "$DOWNLOAD" == "true" ]; then
    curl -s -L -O -J "$DOWNLOAD_URL"
  fi
  ...
  # EOL Spidermonkey versions
  DOWNLOAD_URL="https://archive.mozilla.org/pub/firefox/releases/128.10.0esr/source/firefox-128.10.0esr.source.tar.xz"
  # download old version from Mozilla archive server
  if [ "$DOWNLOAD" == "true" ]; then
    curl -s -L -O -J "$DOWNLOAD_URL"
  fi
